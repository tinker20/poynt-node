...
/**
 * Create a single customer for a business.
 * @param {String} options.businessId
 * @param {Customer} customer
 * @return {Customer} customer
 */
module.exports.createCustomer = function createCustomer(options, customer, next) {
  var hasErr = helpers.hasKeys(options, ['businessId']);
  if (hasErr) { return next(hasErr); }

  this.request({
    url    : '/businesses/' + encodeURIComponent(options.businessId) + '/customers/',
    method : 'POST',
    body: customer
  }, next);
};
